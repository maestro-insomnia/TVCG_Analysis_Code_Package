...
#
# The current implementation is designed for between-subjects data
# with one, two, or three enabled factors. ANOVA and ART-ANOVA always fit the
# complete model implied by all enabled factors: one-factor models contain the
# main effect only, while multi-factor models contain every main effect and
# interaction among all enabled factors. Factor names, levels, dependent variables, categories,
...
sheet_purpose <- function(sheet_name) {
  purposes <- c(
    ...
    `04_Design_Cell_Counts` = "Observation counts for every configured design cell, including empty cells.",
...
analyze_outcome <- function(analysis_data, outcome_row, factor_spec, config, output_paths) {
  ...
  # aliased coefficients. Detect empty design cells before model fitting.
  ...
  if (nrow(empty_cells) > 0L) {
    warnings_errors[[length(warnings_errors) + 1L]] <- tibble::tibble(
      ...
      Message = paste0(
        nrow(empty_cells),
        " empty design cell(s) remain after outcome-specific missing-data removal; ",
        "the complete model would be rank deficient."
      )
    )
    return(list(warnings_errors = dplyr::bind_rows(warnings_errors)))
  }

  # Both analysis branches use the same complete fixed-effects model.
  ...
  if (lm_model$rank < length(stats::coef(lm_model)) || stats::df.residual(lm_model) <= 0L) {
    warnings_errors[[length(warnings_errors) + 1L]] <- tibble::tibble(
      DV = outcome_label, Category = category, Stage = "Linear model", Type = "Error",
      Message = paste0(
        "The fitted model is rank deficient or has no residual degrees of freedom ",
  ...
  if (selected_method == "ANOVA") {
    ...
    if (isTRUE(config$output$save_art_diagnostics)) {
      ...
      # In a single-factor ART model there are no non-target fixed effects to
      # inspect in an ANOVA of aligned (unranked) responses. ARTool therefore
      # returns a zero-row anova.art object for response = "aligned". Printing
      # that zero-row object can trigger an invalid row.names error in the
      # ARTool 0.11.2 print method. Treat this diagnostic as not applicable
      # instead of allowing a diagnostic-only step to abort the analysis.
      aligned_anova_capture <- tryCatch(
        stats::anova(art_model, response = "aligned"),
        error = function(e) e
      )
      aligned_anova_text <- if (inherits(aligned_anova_capture, "error")) {
        paste0("Unavailable: ", conditionMessage(aligned_anova_capture))
      } else if (nrow(aligned_anova_capture) == 0L) {
        paste0(
          "Not applicable for a single-factor ART model: ",
          "there are no non-target fixed effects to test on aligned responses."
        )
      } else {
        tryCatch(
      ...
      if (inherits(diagnostic_write, "error")) {
        ...
        diagnostic_relative_path <- fs::path_rel(
          diagnostic_file, start = output_paths$root
        )
      }
    }
    model_summary <- tibble::tibble(
...
run_analysis <- function(config_file) {
  ...
  run_info <- tibble::tibble(
    ...
    Value = c(
      ...
      paste0("Complete model: Y ~ ", paste(factor_codes, collapse = " * ")),
